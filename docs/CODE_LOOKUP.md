...
- 설명: 9채널 sparse 입력을 받아 32개 residual block을 통과시킨 뒤 CFR 평균과
...
- 파일: `src/models/uacp_predictor.py`
- 함수/부분: `psi_head`
- 설명: backbone feature에서 frequency별 raw 값을 만들고
...
- 설명: 전체 frequency feature를 pooling한 뒤 antenna pair마다 scalar 하나를
...
- 설명: `Psi/(nu-2K-1)`로 계산합니다. 데이터 자체의 예측 어려움을 나타내는
  값으로 논문 Eq.7과 연결됩니다.
...
- 설명: Aleatoric을 Kappa로 나눕니다. evidence가 부족할 때 커지는 모델 불확실성으로
  논문 Eq.8과 연결됩니다.
...
- 설명: pair 하나의 2048차원 CFR를 하나의 multivariate Student-t로 보고
...
- 설명: `||h-gamma||^2*(kappa+nu)`를 사용해 많이 틀린 예측에 높은 evidence를
...
- 설명: pair별 Real/Imag trace를 subcarrier score로 만들고, 마지막에는
...
- 설명: active diagonal Student-t 경로는 식과 맞지만, full Psi를 diagonal로 줄인
...
- 설명: 이후 진단은 이 checkpoint를 읽기만 하며, historical pre-fix STEP 6 checkpoint와
  구분합니다.
