...
- 한 문장 요약: “먼저 각 sample의 실제 채널 응답을 만듭니다.”
...
- 한 문장 요약: “모델이 어느 주파수를 보았는지 알 수 있도록 값과 mask를 같이 줍니다.”
...
- 한 문장 요약: “Sparse CFR와 mask를 받아 전체 CFR와 uncertainty를 동시에 예측합니다.”
...
- 한 문장 요약: “예측 평균뿐 아니라 예측 분포의 크기까지 target CFR에 맞추도록 학습합니다.”
...
- 하는 일: 틀린 예측에 높은 evidence를 주지 않도록 전체 pair error와 evidence를
  함께 벌점으로 줍니다.
- 한 문장 요약: “많이 틀린데 자신만만한 출력을 줄이는 항입니다.”
...
- 한 문장 요약: “데이터 자체의 어려움과 모델이 낯선 상황을 두 값으로 나눕니다.”
...
- 한 문장 요약: “다음에 보고하지 않은 주파수에서 얼마나 위험한지를 계산합니다.”
...
- 흐름: input 생성 -> predictor forward -> NLL와 regularizer 계산 -> backward ->
  `optimizer.step()`
- 한 문장 요약: “예측하고, 틀린 정도를 계산하고, 그 차이를 줄이는 방향으로 weight를 업데이트합니다.”
...
- 한 문장 요약: “학습은 고정하고 환경 난이도만 바꾸어 error와 uncertainty가 함께 움직이는지 봅니다.”
